...
--
-- check_palavras.txt => case INSENSITIVE
-- marcadores.txt     => case SENSITIVE (deve casar exatamente como escrito)
...
local WORDS_FILE   = root .. "/check_palavras/check_palavras.txt"
local MARKERS_FILE = root .. "/check_palavras/marcadores.txt"
...
-- Busca todas as ocorrências de `term` em `line`, respeitando fronteira de palavra.
-- Se case_sensitive == false, ignora maiúsculas/minúsculas na comparação.
-- Se case_sensitive == true, exige correspondência exata de caixa.
local function find_all(line, term, case_sensitive)
    local results = {}
    local search_line = case_sensitive and line or line:lower()
    local search_term = case_sensitive and term or term:lower()
    local pat = escape_pattern(search_term)
    local init = 1
    while true do
        local s, e = search_line:find(pat, init, false)
        if not s then break end
        local before = s > 1 and search_line:sub(s - 1, s - 1) or ""
        local after  = e < #search_line and search_line:sub(e + 1, e + 1) or ""
    ...
    for line in f:lines() do
        ...
        if not is_comment then
            ...
            for _, term in ipairs(forbidden) do
                for _, hit in ipairs(find_all(line, term, false)) do
                    table.insert(issues, {
        ...
        -- marcadores.txt: pegam em comentário também, case sensitive
        for _, term in ipairs(markers) do
            for _, hit in ipairs(find_all(line, term, true)) do
                table.insert(issues, {
                    file = path, line = lineno, col = hit.col,
                    sev = "warning",
...
if #issues == 0 then
    ...
    print(string.format("lint-tex: %d ocorrencia(s).", #issues))
end
